...
class PullRequest(object):
    ...
    def push(self, repodir, labels, confirm=True):
        print(str(self))

        if confirm:
            answer = ''
            while answer not in ['y', 'n']:
                answer = input('Push the Pull Request? [y/n] ').lower()
            
            if answer == 'n':
                return
        
        
        sh = Shell(cwd=repodir, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        # Close PR
        self.comment(diff)
        self.add_label(labels.pushed)
        self.remove_label(labels.accepted)
        self.remove_label(labels.ready)
        self.close()
