...
class Shell(object):
    ...
    def __init__(self, cwd=None, env=None, clear_env=False, shell='/bin/bash', **kwargs):
        self.cwd = cwd
    ...
    def run(self, script, clear_env=False, **kwargs):
        ...
        args = self.__process_args(clear_env, newkwargs)
        command = self.__get_command(script)

        env = self.__get_ordered(newkwargs.get('env', {}))
        if not clear_env:
            env.update(self.env)
